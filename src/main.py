...
class BaseStationApp(App):
    ...
    def build(self):
        ...
        if self.initial_config:
            Clock.schedule_once(lambda dt: self.apply_initial_config(self.initial_config), 0)
    ...
    def apply_initial_config(self, config):
        ...
        if config['USE_FIXED_MODE']:
            ...
            for loc in self.location_data:
                if (abs(loc['latitude'] - coords['LATITUDE']) < 1e-6 and
                    abs(loc['longitude'] - coords['LONGITUDE']) < 1e-6 and
                    abs(loc['altitude'] - coords['ALTITUDE']) < 0.1):
    ...
    def update_ui_with_config(self, coords):
        ...
        self.root.ids.selected_latitude_label.text = f"Latitude: {coords['LATITUDE']:.8f}"
        self.root.ids.selected_longitude_label.text = f"Longitude: {coords['LONGITUDE']:.8f}"
        self.root.ids.selected_altitude_label.text = f"Altitude: {coords['ALTITUDE']:.2f}"
        
        # If we found a matching known location, update the name
        if self.selected_location:
            self.root.ids.selected_name_label.text = f"Name: {self.selected_location['name']}"
    ...
    def show_location_selection(self):
        ...
        for loc_name in self.known_locations:
            ...
            select_btn.bind(on_release=lambda x, ln=loc_name: self.on_location_selected(ln, popup))
            ...
            delete_btn.bind(on_release=lambda x, ln=loc_name: self.confirm_delete_location(ln))
            ...
            with open(json_path, 'r') as f:
                ...
                for loc in data['locations']:
                    if loc['name'] == location_name:
                        ...
                        self.root.ids.selected_name_label.text = f"Selected Location: {loc['name']}"
                        self.root.ids.selected_latitude_label.text = f"Latitude: {loc['latitude']:.10f}"
                        self.root.ids.selected_longitude_label.text = f"Longitude: {loc['longitude']:.10f}"
                        self.root.ids.selected_altitude_label.text = f"Altitude: {loc['altitude']:.2f} m"
                        break
        except Exception as e:
    ...
    def on_save_new_location(self):
        ...
        # Add buttons
        button_box = BoxLayout(orientation='horizontal', size_hint_y=None, height='48dp', spacing=10)
        ...
        def save_location(instance):
            ...
            try:
                ...
                alt = float(self.root.ids.altitude_label.text.split(': ')[1].split(' ')[0])
    ...
    def on_apply_location(self):
        ...
        try:
            ...
            uid_cmd = subprocess.run(['id', '-u', 'adminfarmng'], 
            ...
            subprocess.run([
                ...
                'systemctl', '--user', 'restart', 'farmng-gps.service'
            ], check=True)
            ...
            success_popup.open()
            Clock.schedule_once(lambda dt: success_popup.dismiss(), 2)  # Auto-dismiss after 2 seconds
    ...
    def confirm_delete_location(self, location_name):
        ...
        confirm_btn = Button(
            text='Delete',
            background_color=(1, 0.3, 0.3, 1)  # Red
        )
        ...
        def do_delete(instance):
            try:
                ...
                # Remove location
                data['locations'] = [loc for loc in data['locations'] 
                                if loc['name'] != location_name]
    ...
    async def update_gnss_status(self) -> None:
        ...
        while True:
           ...
           if status:
                print(f"Position: {status.latitude:.10f}, {status.longitude:.10f}, {status.altitude:.5f}")
                # Update Kivy UI with the new GNSS status
                self.root.ids.latitude_label.text = f"Latitude: {status.latitude:.10f}"
                self.root.ids.longitude_label.text = f"Longitude: {status.longitude:.10f}"
